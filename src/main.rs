...
fn main() {
    ...
    let result = if expand > 0 {
        ...
        tilth::run_full(&query, &scope, cli.section.as_deref(), cli.budget, cli.limit, cli.offset, &cache)
    } else {
        tilth::run(&query, &scope, cli.section.as_deref(), cli.budget, cli.limit, cli.offset, &cache)
    };
